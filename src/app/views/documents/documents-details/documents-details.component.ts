...
export class DocumentsDetailsComponent implements OnInit {
 ...
 //   alert(this.documentype)
  //  alert(size);

    if (size <= 3000) {
      if ((this.contentType == '.jpg' || this.contentType == '.jpeg') && this.documentype == 'PHT') {
        if (size <= 15) {
          ...
          this.toastr.error("File Size maximum 15KB only allowed");
      }
      else if (this.contentType == '.pdf' && this.documentype == 'PHT')
        this.toastr.error("Upload Only jpj jpeg file on this category");
      else if ((this.contentType == '.jpg' || this.contentType == '.jpeg') && this.documentype != 'PHT')
        this.toastr.error("Upload Only pdf file on this category");
      else {
        var reader = new FileReader();
        reader.onload = this._handleReaderLoaded.bind(this);
        reader.readAsBinaryString(f);
      ...
      this.toastr.error("File Size maximum 3Mb only allowed");
    }
